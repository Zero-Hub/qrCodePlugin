...
公网代理地址（如 `https://xxxx.dpdns.org`），路径与查询参数原样保留。
...
2. **配置映射规则**：点弹窗右上角齿轮 → 打开配置页 → 添加 / 编辑规则 → 保存。
   ...
   - 例：源 `http://127.0.0.1:5500` → 目标 `https://xxxxx.dpdns.org`
     时，访问 `http://127.0.0.1:5500/foo?x=1` 会生成 `https://xxxxx.dpdns.org/foo?x=1` 的二维码。
